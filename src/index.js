...
const FREE_CHAT_VOICE_CHANNEL_IDS = Object.keys(LISTEN_ONLY_PAIRS);
const ALWAYS_VISIBLE_LISTEN_ONLY_CHANNEL_ID = LISTEN_ONLY_PAIRS[FREE_CHAT_VOICE_CHANNEL_IDS[0]];
const SHARED_LISTEN_ONLY_CHANNEL_ID = '1519364370923126905';
const CONDITIONAL_VOICE_CHANNEL_IDS = ['1519331581410676846', '1519331551278797083'];
const TTS_MIN_VALUE = 0.5;
const TTS_MAX_VALUE = 2;
...
async function syncListenOnlyChannels(guild) {
  ...
  for (const [voiceChannelId, textChannelId] of Object.entries(LISTEN_ONLY_PAIRS)) {
    ...
    activeFreeChats[voiceChannelId] = active;
    if (active) anyFreeChatActive = true;
    await setListenOnlyChannelVisibility(
      guild,
      textChannelId,
      textChannelId === ALWAYS_VISIBLE_LISTEN_ONLY_CHANNEL_ID || active,
    );
  }
  await setListenOnlyChannelVisibility(guild, SHARED_LISTEN_ONLY_CHANNEL_ID, anyFreeChatActive);

  await setEveryoneChannelVisibility(guild, FREE_CHAT_VOICE_CHANNEL_IDS[0], true, {
    visibleReason: 'フリーチャット①は常時表示',
  });
  await setEveryoneChannelVisibility(guild, FREE_CHAT_VOICE_CHANNEL_IDS[1], Boolean(activeFreeChats[FREE_CHAT_VOICE_CHANNEL_IDS[0]]), {
    forceHiddenWhenFalse: true,
    visibleReason: 'フリーチャット①が使用中のためフリーチャット②を表示',
    hiddenReason: 'フリーチャット①が未使用のためフリーチャット②を非表示',
  });
  await setEveryoneChannelVisibility(guild, FREE_CHAT_VOICE_CHANNEL_IDS[2],
    Boolean(activeFreeChats[FREE_CHAT_VOICE_CHANNEL_IDS[0]] && activeFreeChats[FREE_CHAT_VOICE_CHANNEL_IDS[1]]), {
      forceHiddenWhenFalse: true,
      visibleReason: 'フリーチャット①②が使用中のためフリーチャット③を表示',
      hiddenReason: 'フリーチャット①②が未使用のためフリーチャット③を非表示',
    });
  ...
  for (const channelId of CONDITIONAL_VOICE_CHANNEL_IDS) {
    const channel = await guild.channels.fetch(channelId).catch(() => null);
    const channelActive = channel?.isVoiceBased() && channel.members.some((member) => !member.user.bot);
    await setEveryoneChannelVisibility(guild, channelId, Boolean(channelActive || anyFreeChatActive), {
      forceHiddenWhenFalse: true,
      visibleReason: '対象VCまたはフリーチャットが使用中のため表示',
      hiddenReason: '対象VCとフリーチャットが未使用のため非表示',
    });
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  try {
    if (oldState.channelId !== newState.channelId) {
      const visibilityChannelIds = [
        ...FREE_CHAT_VOICE_CHANNEL_IDS,
        ...CONDITIONAL_VOICE_CHANNEL_IDS,
      ];
      if (visibilityChannelIds.includes(oldState.channelId) || visibilityChannelIds.includes(newState.channelId)) {
        await syncListenOnlyChannels(oldState.guild);
      }
