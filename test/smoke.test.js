...
test('VALORANT試合後パーティDM本文を生成できる', () => {
  const text = valorantPartyDmText({
    name: 'PlayerA',
    tag: 'JP1',
    puuid: 'puuid-a',
  }, {
  ...
  assert.match(text, /VALORANT試合後パーティ情報/);
...
test('募集者専用のキャンセルボタンを生成できる', () => {
  ...
  assert.equal(row.components[0].custom_id, 'recruit-complete:123456789012345678');
  assert.equal(row.components[0].label, '募集終了');
  assert.equal(row.components[1].custom_id, 'recruit-cancel:123456789012345678');
  assert.equal(row.components[1].label, '募集キャンセル');
  assert.equal(row.components[2].custom_id, 'recruit-voice:123456789012345678');
...
test('定員到達後も限定VCを開始できる', () => {
  ...
  assert.equal(canEnableLimitedVoice({ closed: true, closedReason: 'completed' }), true);
...
test('個室VC設定パネルに鍵と招待と譲渡をまとめて表示できる', () => {
  const rows = privateRoomSettingsComponents({
    channelId: 'room',
    locked: false,
    invitedUserIds: [],
  }).map((row) => row.toJSON());
  assert.equal(rows[0].components[0].custom_id, 'private-room:name:room');
  assert.equal(rows[0].components[1].custom_id, 'private-room:limit:room');
  assert.equal(rows[0].components[2].custom_id, 'private-room:lock:room');
  assert.equal(rows[0].components[2].label, '鍵をかける');
  assert.equal(rows[1].components[0].custom_id, 'private-room:invite:room');
  assert.equal(rows[2].components[0].custom_id, 'private-room:transfer:room');

  const lockedRows = privateRoomSettingsComponents({
    channelId: 'room',
    locked: true,
    invitedUserIds: [],
  }).map((row) => row.toJSON());
  assert.equal(lockedRows[0].components[2].label, '鍵を開ける');
});
